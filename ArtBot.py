...
DB_CONFIG = {
    'user': 'ArtBasilioBot',
    'password': 'pP92u1RTz!!mfiTzS5t',
    'host': 'localhost',
    'database': 'telegram_bot'
}

# Конфигурация
ADMIN_IDS = [709852580]  # Замените на ID администраторов
CHANNEL_ID = "@ArtBasilioLife"  # Замените на username или ID вашего канала
YOUR_CHAT_ID = 709852580  # Замените на ваш chat_id для уведомлений
...
async def get_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO clients (user_id, info) VALUES (%s, %s)", (user_id, user_info))
    conn.commit()
    cursor.close()
    conn.close()
...
async def clients(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    conn = mysql.connector.connect(**DB_CONFIG)
    cursor = conn.cursor()
    cursor.execute("SELECT id, info FROM clients")
...
# Основная функция
def main() -> None:
    application = Application.builder().token("7619353304:AAGLJS-8WevcUkZIDaWWM5SwveufSvdYszo").build()
